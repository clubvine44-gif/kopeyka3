...
let sb=null,ready=false,saving=false,loading=false,currentUser=null,lastSent='',saveTimer=null,retryTimer=null,suppressSave=false;
...
function waitAppReady(){return new Promise(function(ok){if(window.__FIN_APP_READY&&!window.__FIN_LOAD_PENDING)return ok();var done=false;function fin(){if(done)return;done=true;ok();}try{window.addEventListener('fin-app-ready',fin,{once:true});}catch(e){}var n=0;var t=setInterval(function(){n++;if((window.__FIN_APP_READY&&!window.__FIN_LOAD_PENDING)||n>=80){clearInterval(t);fin();}},100);});}
...
async function onSession(session){if(!session||!session.user){currentUser=null;ready=false;updateAccountUI();return;}if(loading)return;loading=true;try{await waitAppReady();currentUser=session.user;ready=true;updateAccountUI();if(window.__FIN_DECRYPT_FAILED){
...
window.kopeykaCloud={save:function(){return syncNow();},user:function(){return currentUser;},scheduleSave:scheduleSave,load:function(){return loadFromCloud();},showAuth:showAuth,markLocalReset:function(){setClearIntent();},
  threeWay:threeWay,isEmptyState:isEmptyState,
