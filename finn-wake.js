...
function openFinn(cmd){
  ...
  try{
    if(window.Finn3D&&typeof window.Finn3D.activate==='function')window.Finn3D.activate(cmd||null);
    else if(window.kopeykaAssistant&&typeof window.kopeykaAssistant.open==='function')window.kopeykaAssistant.open({command:cmd||null,listen:!cmd});
  }catch(e){}
...
function start(){
  ...
  try{
    ...
    rec.onresult=function(e){for(var i=e.resultIndex;i<e.results.length;i++){var r=e.results[i],text=(r[0]&&r[0].transcript)||'';if(!hasWake(text))continue;var cmd=stripWake(text);if(r.isFinal){openFinn(cmd||null);return;}if(!cmd){openFinn(null);return;}}};
...
function init(){
  ...
  var obs=new MutationObserver(function(){if(!dialogOpen()&&wakeOn&&!rec&&!busy)scheduleStart(400);});
