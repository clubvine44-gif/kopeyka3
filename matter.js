/* МАТЕРИЯ v4 — дверь сбоку, точечные зоны, FB2, ambient, title */
(function () {
  'use strict';
  ...
  function playCreak() {
    ...
    try {
      ...
      o.frequency.exponentialRampToValueAtTime(42, t + 0.72);
      ...
      var buf = a.createBuffer(1, Math.floor(a.sampleRate * 0.5), a.sampleRate);
      var d = buf.getChannelData(0);
      for (var i = 0; i < d.length; i++) d[i] = (Math.random() * 2 - 1) * 0.32;
      n.buffer = buf;
      var ng = a.createGain(), nf = a.createBiquadFilter();
      nf.type = 'bandpass'; nf.frequency.value = 360;
      ng.gain.setValueAtTime(0.06, t);
      ng.gain.exponentialRampToValueAtTime(0.0001, t + 0.48);
  ...
  function startAmbient() {
    ...
    try {
      if (a.state === 'suspended') a.resume();
      var master = a.createGain(); master.gain.value = 0.0; master.connect(a.destination);
      // soft fade-in
      master.gain.linearRampToValueAtTime(0.035, a.currentTime + 2.5);
      ambientNodes = { master: master, list: [] };
      // nostalgic pad: minor chord + slow movement
      var freqs = [146.83, 174.61, 220, 261.63, 329.63];
      ...
      // soft high sparkle
      var spark = a.createOscillator(), sg = a.createGain();
      spark.type = 'sine'; spark.frequency.value = 784;
      sg.gain.value = 0.012;
      var slfo = a.createOscillator(), slg = a.createGain();
      slfo.frequency.value = 0.07; slg.gain.value = 0.008;
      slfo.connect(slg); slg.connect(sg.gain);
      spark.connect(sg); sg.connect(master);
      spark.start(); slfo.start();
      ambientNodes.list.push(spark, slfo);
  ...
  var css = ''
    ...
    + '.mt-exit{width:36px;height:36px;border-radius:50%;background:rgba(0,0,0,.4);border:1px solid rgba(255,255,255,.22);color:#f5efe6;font-size:16px;font-family:system-ui,sans-serif}'
    ...
    + '.mt-brand .mt-title-main{display:block;font-family:Georgia,"Palatino Linotype",serif;font-weight:700;font-style:italic;letter-spacing:.32em;font-size:clamp(12px,3.4vw,16px);color:#fff8e6;'
    + 'text-shadow:0 0 3px #fff,0 0 10px rgba(255,220,140,1),0 0 24px rgba(255,180,70,.85),0 0 48px rgba(255,130,30,.5),0 0 80px rgba(255,100,10,.25);'
    + 'animation:mtSun 3s ease-in-out infinite alternate}'
    + '.mt-brand .mt-title-sub{display:block;margin-top:6px;font-size:9px;letter-spacing:.4em;color:rgba(200,220,255,.45);font-family:system-ui,sans-serif;font-style:normal;font-weight:500}'
    + '@keyframes mtSun{from{filter:brightness(.88);opacity:.9}to{filter:brightness(1.2);opacity:1}}'
    ...
    + '.rm-bg{position:absolute;left:0;top:0;width:100%;height:100%;object-fit:cover;object-position:center center;border:0;margin:0;padding:0;display:block;pointer-events:none}'
    ...
    + '.mt-float{position:absolute;left:50%;top:50%;transform:translate(-50%,-48%) scale(.94);opacity:0;pointer-events:none;z-index:10;width:min(340px,88vw);'
    + 'background:rgba(6,8,14,.55);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);border:1px solid rgba(255,220,160,.18);border-radius:20px;padding:20px 18px;color:#f2e8d4;'
    + 'box-shadow:0 20px 60px rgba(0,0,0,.45),inset 0 1px 0 rgba(255,255,255,.06);transition:opacity .35s,transform .4s cubic-bezier(.2,.9,.25,1);font-family:system-ui,sans-serif}'
    + '.mt-float.show{opacity:1;transform:translate(-50%,-50%) scale(1);pointer-events:auto}'
    + '.mt-float h3{font-size:16px;margin:0 0 10px;font-weight:700;color:#ffe9b8;letter-spacing:.02em}'
    + '.mt-float .mt-meta{font-size:13px;color:rgba(220,210,190,.85);line-height:1.5}'
    ...
    + '.mt-float .mt-close{background:rgba(255,255,255,.07);color:#e8e0d0;border:1px solid rgba(255,255,255,.1)}'
    + '.mt-float .mt-act{background:linear-gradient(135deg,rgba(229,167,94,.9),rgba(240,192,96,.95));color:#1a1208}'
    ...
    + '.mt-goalrow{display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid rgba(255,255,255,.07);font-size:13px}'
    + '.mt-goalrow b{color:#ffd9a8}'
    + '#mtReader{position:absolute;inset:0;display:none;z-index:20;background:#0c0a08;flex-direction:column;font-family:Georgia,"Palatino Linotype",serif}'
    + '#mtReader.show{display:flex}'
    ...
    + '.rd-top button{width:36px;height:36px;border-radius:50%;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);color:#e8e0d0;font-size:15px}'
    + '.rd-top .rd-name{flex:1;min-width:0;font-size:13px;color:#f0e6d0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}'
    + '.rd-progress{height:3px;background:rgba(255,255,255,.08);margin:0 14px 8px;border-radius:99px;overflow:hidden}'
    + '.rd-progress i{display:block;height:100%;background:linear-gradient(90deg,#E5A75E,#F0C060);border-radius:99px;transition:width .2s}'
    + '.rd-body{flex:1;overflow:hidden;position:relative;padding:0 18px}'
    + '.rd-page{height:100%;overflow-y:auto;-webkit-overflow-scrolling:touch;color:#e8e0d0;font-size:17px;line-height:1.65;padding:8px 0 24px;white-space:pre-wrap}'
    ...
    + '.rd-nav button{flex:1;padding:12px;border-radius:12px;border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.06);color:#e8e0d0;font-weight:600;font-size:14px;font-family:system-ui,sans-serif}'
    + '.rd-nav .rd-info{flex:0 0 auto;min-width:70px;text-align:center;font-size:12px;color:rgba(200,190,170,.7);align-self:center;font-family:system-ui,sans-serif}'
    + '.rd-lib{padding:12px 14px;overflow-y:auto;flex:1}'
    + '.rd-item{padding:12px 14px;margin-bottom:8px;border-radius:14px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08)}'
    + '.rd-item b{display:block;font-size:14px;color:#f0e6d0;margin-bottom:6px}'
    + '.rd-bar{height:4px;background:rgba(255,255,255,.08);border-radius:99px;overflow:hidden;margin-top:4px}'
    + '.rd-bar i{display:block;height:100%;background:#E5A75E;border-radius:99px}'
  ...
  /* hotspot map — точечно под референс комнаты (cover, center) */
  var HOTSPOTS = [
    { id: 'bed',    style: 'left:1%;top:38%;width:26%;height:30%' },
    { id: 'window', style: 'left:30%;top:12%;width:36%;height:20%' },
    { id: 'desk',   style: 'left:28%;top:40%;width:36%;height:14%' },
    { id: 'goals',  style: 'right:5%;top:14%;width:18%;height:14%' },
    { id: 'piggy',  style: 'right:7%;top:36%;width:16%;height:11%' },
    { id: 'plant',  style: 'right:12%;bottom:18%;width:20%;height:20%' },
    { id: 'book',   style: 'left:34%;bottom:12%;width:26%;height:13%' },
    { id: 'diary',  style: 'left:2%;bottom:1%;width:28%;height:16%' },
    { id: 'lamp',   style: 'right:2%;bottom:2%;width:14%;height:12%' },
    { id: 'door',   style: 'right:0%;top:24%;width:13%;height:46%' }
  ];

  var root = document.createElement('div');
  root.id = 'matterRoot';
  var hotHtml = HOTSPOTS.map(function (h) {
    return '<button type="button" class="rm-hot" data-obj="' + h.id + '" style="' + h.style + '" aria-label="' + h.id + '"></button>';
  }).join('');
  root.innerHTML =
    ...
    '<div id="mtReader">' +
      ...
      '<div class="rd-nav" id="rdNav" hidden>' +
        '<button type="button" id="rdPrev">← Назад</button>' +
        '<span class="rd-info" id="rdInfo">1 / 1</span>' +
        '<button type="button" id="rdNext">Далее →</button>' +
      '</div>' +
  ...
  function loadRoomImage() {
    ...
    if (!img) return;
    ['matter-room.jpg', 'matter-room.png', 'assets/matter-room.jpg'].forEach(function (u, idx) {
  ...
  function resize() {
    ...
    if (canvas.width !== bw || canvas.height !== bh2) {
      canvas.width = bw; canvas.height = bh2;
  ...
  function seedSpace() {
    bgStars = [];
    var nStars = Math.min(110, Math.floor((W * H) / 9500));
    for (var i = 0; i < nStars; i++) {
      bgStars.push({ x: Math.random() * W, y: Math.random() * H, r: 0.4 + Math.random() * 1.25, ph: Math.random() * 6.28, sp: 0.25 + Math.random() * 0.6 });
    }
    goals = buildGoals();
    var cx = W * 0.42, cy = H * 0.48, n = goals.length;
    goals.forEach(function (g, i) {
      var ang = -Math.PI * 0.6 + (i / Math.max(1, n - 1)) * Math.PI * 1.05;
      var rad = Math.min(W, H) * 0.2 + ((hash(g.id) % 36));
      g.x = cx + Math.cos(ang) * rad * 1.1;
      g.y = cy + Math.sin(ang) * rad * 0.7;
      g.hue = g.type === 'dream' ? 42 : 200;
      g.pulse = Math.random() * 6;
    });
    // дверь сбоку справа, отдельно от целей
    door.x = W * 0.86;
    door.y = H * 0.58;
    door.open = 0;
    door.opening = false;
  ...
  function enterMatter() {
    ...
    particles = [];
    var np = Math.min(60, Math.floor(W / 9));
    for (var i = 0; i < np; i++) particles.push({ a: Math.random() * Math.PI * 2, r: 20 + Math.random() * 48, sp: 0.85 + Math.random() * 1.8, s: 1 + Math.random() * 2 });
  ...
  function loop(ts) {
    ...
    rafId = requestAnimationFrame(loop);
    var dt = Math.min(0.048, (ts - lastT) / 1000); lastT = ts;
  ...
  function drawBlackHole(dt) {
    ...
    grd.addColorStop(0, '#000'); grd.addColorStop(0.55, '#050810');
    grd.addColorStop(0.82, 'rgba(30,50,100,0.35)'); grd.addColorStop(1, 'rgba(0,0,0,0)');
    ...
    ctx.lineWidth = 1.4; ctx.arc(cx, cy, bh.r * 0.38, 0, Math.PI * 2); ctx.stroke();
    if (bh.fall > 0) {
      var v = Math.min(1, bh.fall / 1.25);
      ctx.fillStyle = 'rgba(0,0,0,' + v + ')'; ctx.fillRect(0, 0, W, H);
  ...
  function drawSpace(dt, ts) {
    ...
    for (var i = 0; i < bgStars.length; i++) {
      var s = bgStars[i];
      ctx.globalAlpha = 0.2 + Math.abs(Math.sin(t * s.sp + s.ph)) * 0.55;
      ctx.fillStyle = '#fff';
      ctx.beginPath(); ctx.arc(s.x, s.y, s.r, 0, Math.PI * 2); ctx.fill();
    }
    ctx.globalAlpha = 1;
    ctx.strokeStyle = 'rgba(140,180,255,0.12)'; ctx.lineWidth = 1;
  ...
  function drawConstellationDoor(t) {
    ...
    var sc = 1 + door.open * 0.28;
    var hw = 18 * sc, hh = 32 * sc;
    var pg = ctx.createRadialGradient(dx, dy, 0, dx, dy, hh * 1.4);
    pg.addColorStop(0, 'rgba(120,170,255,' + (0.14 + door.open * 0.28) + ')');
    pg.addColorStop(1, 'rgba(20,40,80,0)');
    ctx.fillStyle = pg; ctx.beginPath(); ctx.arc(dx, dy, hh * 1.4, 0, Math.PI * 2); ctx.fill();
    var pts = [
      [-hw, hh], [-hw, -hh * 0.5], [-hw * 0.5, -hh], [0, -hh * 1.15], [hw * 0.5, -hh], [hw, -hh * 0.5], [hw, hh]
    ];
    ctx.strokeStyle = 'rgba(180,210,255,' + (0.4 + 0.35 * door.open) + ')';
    ctx.lineWidth = 1.3;
    ctx.beginPath();
    for (var i = 0; i < pts.length; i++) {
      var px = dx + pts[i][0], py = dy + pts[i][1];
      if (i === 0) ctx.moveTo(px, py); else ctx.lineTo(px, py);
    }
    ctx.closePath(); ctx.stroke();
    if (door.open < 0.65) {
      ctx.globalAlpha = 1 - door.open;
      ctx.beginPath(); ctx.moveTo(dx, dy - hh * 0.85); ctx.lineTo(dx, dy + hh); ctx.stroke();
      ctx.globalAlpha = 1;
    }
    for (var j = 0; j < pts.length; j++) {
      var qx = dx + pts[j][0] * (1 - door.open * 0.12);
      var qy = dy + pts[j][1];
      var pr = 2.1 * (0.85 + 0.15 * Math.sin(t * 2 + j));
      ctx.beginPath(); ctx.fillStyle = 'rgba(255,245,220,' + (0.7 + 0.3 * Math.sin(t * 1.5 + j)) + ')';
      ctx.arc(qx, qy, pr, 0, Math.PI * 2); ctx.fill();
      ctx.beginPath(); ctx.fillStyle = 'rgba(160,200,255,0.22)'; ctx.arc(qx, qy, pr * 2.8, 0, Math.PI * 2); ctx.fill();
    }
  }
  ...
  canvas.addEventListener('click', function (ev) {
    ...
    for (var i = 0; i < goals.length; i++) {
      var s = goals[i], d = (x - s.x) * (x - s.x) + (y - s.y) * (y - s.y);
      if (d < bestD) { bestD = d; best = s; }
    }
    if (best) {
      var pct = Math.round((best.progress || 0) * 100);
      showFloat('<h3>' + best.title + '</h3><p class="mt-meta">Прогресс: ' + pct + '% · Осталось: ' + (best.remain || '—') + '</p><button type="button" class="mt-close" id="mtFClose">Закрыть</button>');
    }
  }, { passive: true });

  function showFloat(html) {
    var f = document.getElementById('mtFloat');
    document.getElementById('mtFloatBody').innerHTML = html;
    f.classList.add('show');
    var c = document.getElementById('mtFClose');
    if (c) c.onclick = hideFloat;
  }
  ...
  function onObj(id) {
    ...
    if (id === 'piggy') {
      showFloat('<h3>Копилка</h3><p class="mt-meta">В резервах: <b style="color:#ffd9a8">' + readReserveTotal().toLocaleString('ru-RU') + ' ₽</b><br><br>Отражение Финны — здесь ничего не списывается.</p><button type="button" class="mt-close" id="mtFClose">Закрыть</button>');
      return;
    }
    if (id === 'plant') {
    ...
    if (id === 'diary' || id === 'desk') {
      ...
      showFloat('<h3>Дневник</h3><textarea id="mtDiaryInput" placeholder="Мысли…">' + last + '</textarea><button type="button" class="mt-act" id="mSaveDiary">Сохранить</button><button type="button" class="mt-close" id="mtFClose">Закрыть</button>');
      setTimeout(function () {
    ...
    if (id === 'goals') {
      ...
      showFloat('<h3>Цели</h3>' + rows + '<button type="button" class="mt-close" id="mtFClose">Закрыть</button>');
      return;
    }
    if (id === 'window') {
      showFloat('<h3>Окно</h3><p class="mt-meta">Город внизу дышит огнями.<br>Ночь держит тишину — как будто время чуть замедлилось.</p><button type="button" class="mt-close" id="mtFClose">Закрыть</button>');
      return;
    }
    if (id === 'bed') {
      showFloat('<h3>Кровать</h3><p class="mt-meta">Место, где можно просто выдохнуть.<br>Вечерний режим — позже.</p><button type="button" class="mt-close" id="mtFClose">Закрыть</button>');
      return;
    }
    if (id === 'lamp') {
      showFloat('<h3>Свет</h3><p class="mt-meta">Тёплый огонёк. Комната живёт своей жизнью.</p><button type="button" class="mt-close" id="mtFClose">Закрыть</button>');
      return;
    }
  }
  ...
  function openReader() {
    ...
    document.getElementById('rdNav').hidden = true;
  ...
  function closeReader() {
    ...
    if (reader.bookId != null && reader.pages.length) {
      ...
      for (var i = 0; i < list.length; i++) {
        if (list[i].id === reader.bookId) {
          list[i].page = reader.page;
          list[i].total = reader.pages.length;
  ...
  function renderLibrary() {
    var box = document.getElementById('rdLib');
    var list = loadBooks();
    var html = '<button type="button" class="rd-add" id="rdAdd">+ Загрузить FB2</button>';
    if (!list.length) html += '<p style="color:rgba(200,190,170,.55);font-size:13px;text-align:center;margin-top:24px">Пока пусто. Загрузи книгу в формате FB2.</p>';
    list.forEach(function (b) {
      var pct = Math.round((b.progress || 0) * 100);
      html += '<div class="rd-item" data-bid="' + b.id + '">' +
        '<b>' + (b.title || 'Без названия') + '</b>' +
        '<span style="font-size:11px;color:rgba(200,190,170,.55)">' + pct + '% прочитано</span>' +
        '<div class="rd-bar"><i style="width:' + pct + '%"></i></div></div>';
    });
    box.innerHTML = html;
    document.getElementById('rdAdd').onclick = function () { document.getElementById('rdFile').click(); };
    box.querySelectorAll('.rd-item').forEach(function (el) {
      el.onclick = function () { openBook(el.getAttribute('data-bid')); };
    });
  }
  ...
  document.getElementById('rdFile').onchange = function (ev) {
    ...
    readerObj.onload = function () {
      try {
        var text = String(readerObj.result || '');
        var parsed = parseFb2(text);
        ...
        // keep raw pages in storage (may be large — store text only)
        saveBooks(list.map(function (b) {
          if (b.id === id) return b;
          // strip pages from others to save space if needed — keep pages for open books
          return b;
        }));
        openBook(id);
      } catch (e) { alert('Ошибка чтения файла'); }
    };
    readerObj.readAsText(f, 'UTF-8');
    ev.target.value = '';
  };
  ...
  function parseFb2(xml) {
    var title = 'Книга';
    try {
      var tm = xml.match(/<book-title[^>]*>([\s\S]*?)<\/book-title>/i);
      if (tm) title = tm[1].replace(/<[^>]+>/g, '').trim() || title;
    } catch (e) {}
    var body = xml;
    try {
      var bm = xml.match(/<body[^>]*>([\s\S]*?)<\/body>/i);
      if (bm) body = bm[1];
    } catch (e) {}
    body = body
      .replace(/<title[\s\S]*?<\/title>/gi, function (m) { return '\n\n' + m.replace(/<[^>]+>/g, '') + '\n\n'; })
      .replace(/<p[^>]*>/gi, '\n')
      .replace(/<\/p>/gi, '\n')
    ...
    var pages = [];
    for (var i = 0; i < body.length; i += charsPerPage) {
      pages.push(body.slice(i, i + charsPerPage));
    }
  ...
  function openBook(id) {
    ...
    reader.mode = 'read';
    reader.bookId = b.id;
    reader.title = b.title;
    reader.pages = b.pages;
    ...
    document.getElementById('rdNav').hidden = false;
  ...
  function renderPage() {
    ...
    var pct = reader.pages.length ? ((reader.page + 1) / reader.pages.length) * 100 : 0;
    document.getElementById('rdProg').style.width = pct + '%';
    // persist
    var list = loadBooks();
    for (var i = 0; i < list.length; i++) {
      if (list[i].id === reader.bookId) {
  ...
  document.getElementById('rdPrev').onclick = function () {
    if (reader.page > 0) { reader.page--; renderPage(); document.getElementById('rdPage').scrollTop = 0; }
  };
  document.getElementById('rdNext').onclick = function () {
    if (reader.page < reader.pages.length - 1) { reader.page++; renderPage(); document.getElementById('rdPage').scrollTop = 0; }
  };
