...
function shift(ds,ov){
  var v=ov&&ov[ds];
  if(typeof v==='string'&&SHIFT_LABEL[v])return v;
  return CYCLE[((days(ANCHOR,ds)%6)+6)%6];
}
function cleanShifts(ov){
  var out={};
  if(!ov||typeof ov!=='object')return out;
  Object.keys(ov).forEach(function(k){
    var v=ov[k];
    if(typeof v==='string'&&SHIFT_LABEL[v])out[k]=v;
  });
  return out;
}
...
function render(){
  ...
  var resH=STATE.reserves.length?STATE.reserves.map(function(r){return '<div class="item" data-id="'+r.id+'" data-k="res"><div class="left"><b>'+esc(r.name)+'</b><span class="muted">'+fmt(r.saved)+(r.target?' / '+fmt(r.target):'')+'</span></div></div>';}).join(''):'<div class="empty">Резервов нет</div>';
