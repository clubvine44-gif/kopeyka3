...
function setEmotion(root, emotion){
  ...
  var browColor = angry ? '#F87171' : (emotion==='happy' ? '#FFD9A0' : '#F2C879');
  var browXform = angry ? {l:'rotate(-16deg) translateY(2px)', r:'rotate(16deg) translateY(2px)'}
    : emotion==='happy' ? {l:'translateY(-1.5px)', r:'translateY(-1.5px)'}
    : emotion==='thinking' ? {l:'rotate(-4deg)', r:'rotate(2deg) translateY(-1px)'}
    : {l:'', r:''};
  if(browL){ browL.style.opacity=angry?'1':'.85'; browL.style.transform=browXform.l; browL.setAttribute('stroke',browColor); }
  if(browR){ browR.style.opacity=angry?'1':'.85'; browR.style.transform=browXform.r; browR.setAttribute('stroke',browColor); }
