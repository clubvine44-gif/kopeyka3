...
function buildPlan(opts){
  ...
  var plan = { id: 'mp_'+Date.now().toString(36), createdAt: new Date().toISOString(), storeId: 'ogni', storeName: 'Огни', goal: goal, adults: adults, children: children, days: days, budgetMonth: budgetMonth, budgetForPeriod: budgetTotal, total: total, fits: fits, note: note, menu: menu.slice(0, 7), menuFullDays: days, basket: basketList };
...
function mealHtml(){
  var st = loadMealState(); var s = st.settings || {}; var plan = st.lastPlan; var cat = getCatalog(); var html = '';
  html += '<div class="view-header"><button type="button" class="back-btn" data-act="go-home">←</button><h2>Рацион · Огни</h2></div>';
  html += '<div class="card tight"><div class="card-title">Магазин</div><div style="font-weight:700">Огни · Кисловодск</div><div class="muted" style="font-size:12px;margin-top:4px">Нет открытого прайса в сети. Нажми цену в каталоге и поставь как на ценнике — расчёт станет точным.</div></div>';
  ...
  [['maintain','Питаться как обычно'],['lose','Похудеть'],['gain','Набрать массу']].forEach(function(o){ html += '<option value="'+o[0]+'"'+(s.goal===o[0]?' selected':'')+'>'+o[1]+'</option>'; });
  ...
  if(plan){
    ...
    html += '<div class="muted" style="font-size:12px;margin:6px 0 10px">'+(plan.note||'')+'</div><div class="card-title">Корзина</div><div class="list">';
  ...
  html += '<div class="card tight"><div class="card-title">Каталог · цены Огни</div><div class="list">';
  ...
  html += '</div></div><div class="hint">Открыть: goView("meal") в консоли. Кнопку на главный экран добавим в следующем шаге.</div>';
  return html;
}
function bindMealUI(root){
  ...
  if(build) build.onclick = function(){
    var plan = buildPlan({ budgetMonth: num((root.querySelector('#mealBudget')||{}).value), adults: num((root.querySelector('#mealAdults')||{}).value)||1, children: num((root.querySelector('#mealChildren')||{}).value), goal: ((root.querySelector('#mealGoal')||{}).value)||'maintain', days: 30 });
    try{ if(typeof toast==='function') toast(fmt(plan.total)); }catch(e){}
  ...
  root.querySelectorAll('[data-edit-price]').forEach(function(btn){
    btn.onclick = function(){
      var id = btn.getAttribute('data-edit-price'); var p = productById(id);
      var v = prompt('Цена в Огни, ₽', String(p?p.price:0)); if(v==null) return; setPrice(id, v);
      try{ if(typeof goView==='function') goView('meal'); else if(typeof render==='function') render(); }catch(e){}
    };
  });
}
function tryHookApp(){
  ...
  var tries=0; function patchRender(){
    tries++; if(typeof window.render!=='function'){ if(tries<30) setTimeout(patchRender,200); return; }
    if(window.render.__mealPatched) return;
    var orig=window.render;
    window.render=function(){
      try{ if((window.currentView||window.__finView)==='meal'){ var r=orig.apply(this,arguments); setTimeout(function(){ var wrap=document.querySelector('.wrap'); if(wrap){ wrap.innerHTML=mealHtml(); bindMealUI(wrap); } },0); return r; } }catch(e){}
  ...
  document.addEventListener('click', function(ev){
    ...
    if(t.closest('[data-act="open-meal"]')){ ev.preventDefault(); window.currentView='meal'; window.__finView='meal'; if(typeof goView==='function') goView('meal'); else if(typeof render==='function') render(); }
    if(t.closest('[data-act="go-home"]')){ if(typeof goHome==='function') goHome(); else if(typeof goView==='function') goView('home'); }
  }, true);
  console.log('[FINNA meal-plan] Огни');
}
