...
(function (global) {
  ...
  function deriveKey() {
    ...
    return crypto.subtle.importKey('raw', enc.encode(id), 'PBKDF2', false, ['deriveKey']).then(function (base) {
      return crypto.subtle.deriveKey(
        { name: 'PBKDF2', salt: enc.encode('finna-secure-store-v1'), iterations: 120000, hash: 'SHA-256' },
        base,
  ...
  function loadState(storageKey, defFn, normFn) {
    return init().then(function () {
      var raw = null;
      try { raw = localStorage.getItem(storageKey); } catch (e) { return defFn(); }
      if (!raw) return defFn();
      function parseOk(text) {
        try { return normFn(JSON.parse(text)); } catch (e) { return defFn(); }
      }
  ...
  function saveState(storageKey, stateObj) {
    ...
    return init().then(function () {
      if (!_cryptoKey) {
        try { localStorage.setItem(storageKey, plain); } catch (e) {}
        return false;
      }
      return encryptString(plain).then(function (enc) {
        try { localStorage.setItem(storageKey, enc); } catch (e) {}
        return true;
      }).catch(function () {
        try { localStorage.setItem(storageKey, plain); } catch (e) {}
        return false;
