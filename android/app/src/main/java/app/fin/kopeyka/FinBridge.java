...
public class FinBridge {
    ...
    public void showNotification(String title, String message) {
        ...
        NotificationCompat.Builder b = new NotificationCompat.Builder(context, CHANNEL_ID)
                ...
                .setDefaults(NotificationCompat.DEFAULT_ALL).setSound(sound).setVibrate(new long[]{0, 180, 100, 180})
                .setCategory(NotificationCompat.CATEGORY_REMINDER).setVisibility(NotificationCompat.VISIBILITY_PUBLIC);
    ...
    @JavascriptInterface public void scheduleReminders(String json) {
        try {
            JSONArray arr = new JSONArray(json); SharedPreferences prefs = context.getSharedPreferences("fin_reminders", Context.MODE_PRIVATE); Set<String> oldIds = new HashSet<>(prefs.getStringSet("ids", Collections.<String>emptySet())); Set<String> newIds = new HashSet<>(); AlarmManager am = (AlarmManager) context.getSystemService(Context.ALARM_SERVICE);
            for (int i=0;i<arr.length();i++) { JSONObject o=arr.getJSONObject(i); String id=o.optString("id",""); if(id.isEmpty())continue; String date=o.optString("date",""); String title=o.optString("title","Финн"); String message=o.optString("message",""); long triggerAt=dateToMillis(date); if(triggerAt<=0)continue; newIds.add(id); int reqCode=id.hashCode(); Intent intent=new Intent(context,ReminderReceiver.class); intent.putExtra("title",title); intent.putExtra("message",message); PendingIntent pi=PendingIntent.getBroadcast(context,reqCode,intent,PendingIntent.FLAG_UPDATE_CURRENT|PendingIntent.FLAG_IMMUTABLE); try{if(am!=null)am.setAndAllowWhileIdle(AlarmManager.RTC_WAKEUP,triggerAt,pi);}catch(Exception ignored){} }
