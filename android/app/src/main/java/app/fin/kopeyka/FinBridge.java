...
public class FinBridge {
    ...
    /** Sends the one-time welcome push after the first app launch, once notification permission is available. */
    ...
    @JavascriptInterface public int getVersionCode() {
        try { return context.getPackageManager().getPackageInfo(context.getPackageName(), 0).versionCode; } catch (Exception e) { return 0; }
    }
    ...
    public void showNotification(String title, String message) {
        ...
        try { NotificationManagerCompat.from(context).notify((int) (System.currentTimeMillis() & 0xffff), b.build()); } catch (SecurityException ignored) {}
    }
    ...
    @JavascriptInterface public void saveBackup(String json, String filename) {
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                ...
                cv.put(MediaStore.Downloads.DISPLAY_NAME, filename); cv.put(MediaStore.Downloads.MIME_TYPE, "application/json"); cv.put(MediaStore.Downloads.IS_PENDING, 1);
                ...
                File file = new File(dir, filename); FileOutputStream out = new FileOutputStream(file); out.write(json.getBytes(StandardCharsets.UTF_8)); out.close();
            }
    ...
    @JavascriptInterface public void scheduleReminders(String json) {
        try {
            JSONArray arr = new JSONArray(json); SharedPreferences prefs = context.getSharedPreferences("fin_reminders", Context.MODE_PRIVATE);
            Set<String> oldIds = new HashSet<>(prefs.getStringSet("ids", Collections.<String>emptySet())); Set<String> newIds = new HashSet<>();
            AlarmManager am = (AlarmManager) context.getSystemService(Context.ALARM_SERVICE);
            for (int i=0;i<arr.length();i++) {
                JSONObject o=arr.getJSONObject(i); String id=o.optString("id",""); if(id.isEmpty())continue; String date=o.optString("date",""); String title=o.optString("title","Финн"); String message=o.optString("message",""); long triggerAt=dateToMillis(date); if(triggerAt<=0)continue;
                newIds.add(id); int reqCode=id.hashCode(); Intent intent=new Intent(context,ReminderReceiver.class); intent.putExtra("title",title); intent.putExtra("message",message); PendingIntent pi=PendingIntent.getBroadcast(context,reqCode,intent,PendingIntent.FLAG_UPDATE_CURRENT|PendingIntent.FLAG_IMMUTABLE); try{if(am!=null)am.setAndAllowWhileIdle(AlarmManager.RTC_WAKEUP,triggerAt,pi);}catch(Exception ignored){}
            }
            for(String oldId:oldIds){if(newIds.contains(oldId))continue;int reqCode=oldId.hashCode();Intent intent=new Intent(context,ReminderReceiver.class);PendingIntent pi=PendingIntent.getBroadcast(context,reqCode,intent,PendingIntent.FLAG_UPDATE_CURRENT|PendingIntent.FLAG_IMMUTABLE);if(am!=null)try{am.cancel(pi);}catch(Exception ignored){}}
