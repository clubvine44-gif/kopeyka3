...
function injectStyle(){
  ...
  var s=document.createElement('style'); s.id='onboardStyle';
  s.textContent=[
    '.onboard{position:fixed;inset:0;z-index:2000;background:rgba(5,6,9,.88);display:none;align-items:center;justify-content:center;padding:20px;box-sizing:border-box;backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px)}',
    '.onboard-card{width:100%;max-width:440px;background:linear-gradient(160deg,#191C24,#121419);border:1px solid rgba(229,167,94,.2);border-radius:24px;padding:24px 20px;color:#F2F3F7;box-shadow:0 20px 70px rgba(0,0,0,.55)}',
    '.onboard-brand{display:flex;align-items:center;gap:9px;color:#E5A75E;font-size:12px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;margin-bottom:18px}',
    '.onboard-dot{width:8px;height:8px;border-radius:50%;background:#E5A75E;box-shadow:0 0 16px rgba(229,167,94,.7)}',
    '.onboard-step{font-size:11px;color:#8F96A7;text-transform:uppercase;letter-spacing:.07em;margin-bottom:9px}',
    '.onboard-card h2{font-size:23px;line-height:1.15;margin:0 0 11px;font-weight:750;letter-spacing:-.02em}',
    '.onboard-card p{font-size:14px;line-height:1.58;color:#C8CCD6;margin:0 0 20px}',
    '.onboard-progress{height:4px;background:#272B35;border-radius:99px;overflow:hidden;margin-bottom:18px}',
    '.onboard-progress i{display:block;height:100%;background:linear-gradient(90deg,#F0C384,#E5A75E);border-radius:99px;transition:width .2s}',
    '.onboard-actions{display:flex;gap:9px}',
    '.ob-skip,.ob-next{flex:1;padding:13px;border-radius:13px;font-weight:700;border:0;cursor:pointer;font-size:14px}',
    '.ob-skip{background:#20232C;color:#F2F3F7;border:1px solid rgba(255,255,255,.1)}',
    '.ob-next{background:linear-gradient(135deg,#F0C384,#E5A75E);color:#1A1208;box-shadow:0 7px 22px rgba(229,167,94,.2)}'
  ].join('');
...
function show(step){
  ...
  injectStyle();
  var s=STEPS[onboardStep], root=document.getElementById('onboard');
  if(!root){root=document.createElement('div');root.id='onboard';root.className='onboard';document.body.appendChild(root);}
  var last=onboardStep===STEPS.length-1, pct=Math.round(((onboardStep+1)/STEPS.length)*100);
  root.innerHTML='<div class="onboard-card"><div class="onboard-brand"><span class="onboard-dot"></span>ФИНН</div><div class="onboard-step">Шаг '+(onboardStep+1)+' из '+STEPS.length+'</div><div class="onboard-progress"><i style="width:'+pct+'%"></i></div><h2>'+esc(s.t)+'</h2><p>'+esc(s.b)+'</p><div class="onboard-actions">'+(onboardStep>0?'<button type="button" class="ob-skip" id="obBack">Назад</button>':'<button type="button" class="ob-skip" id="obSkip">Пропустить</button>')+'<button type="button" class="ob-next" id="obNext">'+(last?'Начать пользоваться':'Далее')+'</button></div></div>';
